...
public class digital_clock extends Label {
    ...
    public static boolean backupDataWithDatabase() throws Exception{
        ...
        String host=Check.ipaddress_name();
        String port="3306";
        String user=Check.duser();
        String password=Check.dpword();
        String database=Check.dname();

        Month month = Controller.today.getMonth();
        int year = Controller.today.getYear();
        int day = Controller.today.getDayOfMonth();

        //creating folder
        new File(Check.drive_name()+Configuring_Path.FOLDER_PATH).mkdir();
        new File(Check.drive_name()+Configuring_Path.DATA_BACKUP_PATH).mkdir();
        new File(Check.drive_name()+Configuring_Path.DATA_BACKUP_PATH+year).mkdir();
        new File(Check.drive_name()+Configuring_Path.DATA_BACKUP_PATH+year+"/"+month.name()).mkdir();
        new File(Check.drive_name()+Configuring_Path.DATA_BACKUP_PATH+year+"/"+month.name()+"/"+day).mkdir();


        String backupPath=Check.drive_name()+Configuring_Path.DATA_BACKUP_PATH+year+"/"+month.name()+"/"+day+"/" ;
